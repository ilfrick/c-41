...
// Spline machinery lives in `crate::splines` — shared with the other
// curve-driven IOPs (lowlight). Only the V2 *sampler* is colorzones-specific.
use crate::splines::{
    compute_catmull_rom_tangents, compute_monotone_hermite_tangents,
    compute_monotone_hermite_variant_tangents_periodic, compute_smooth_cubic_tangents,
    eval_knots, Knot, CATMULL_ROM, MONOTONE_HERMITE,
};
