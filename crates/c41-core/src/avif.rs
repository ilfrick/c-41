...
//! u8-to-float normalize loop, m4-210) and from
//! `src/imageio/format/avif.c` (`write_image`, the 10/12-bit float-to-u16
//! export pack and the 8-bit float-to-u8 export pack, m4-216).
//!
//! Import direction: each kernel replaces its whole loop body — every
//! decoded AVIF lane is scaled by `1/max_channel_f` into the 4-channel
//! float mipmap buffer, with the alpha lane zeroed.
//!
//! Export direction (`avif_float_to_u16`, m4-216, and `avif_float_to_u8`,
//! m4-216): each kernel replaces its whole loop body — every float RGB
//! lane is scaled by `max_channel_f`, clamped, and rounded into the
//! libavif RGB plane, with the alpha lane never read. Same
//! multiply-then-branch-clamp-then-round spelling as the HEIF export
//! kernels of m4-213/m4-214, whose C loops are textually identical apart
//! from the plane owner.
//!
...
//!   `AVIF_RGB_FORMAT_RGB`, depth 10 or 12 so every lane is 16 bits wide),
//!   and
//! - `mipbuf[4*(y*width+x) + 3] = 0.0` (alpha zeroed, as in the HEIF
//!   kernel of m4-208).
//!
//! Bit-exactness notes:
...
//!   (stated in `imageio_heif.c` next to its decode call, and libavif emits
//!   host-order lanes the same way), so the kernel decodes each lane with
//!   `u16::from_le_bytes` explicitly rather than relying on the host
//!   endianness.
...
/// Scale decoded AVIF 16-bit lanes into the float mipmap buffer.
///
/// Port of the former element-wise loop in `dt_imageio_open_avif`
/// (src/imageio/imageio_avif.c, the `case 12 / case 10` branch): `src`
/// holds the interleaved RGB plane (`height` rows, `rowbytes` bytes each,
/// 3 little-endian u16 lanes per pixel), `out` receives `4*width*height`
/// floats (RGB lanes times the `1/max_channel` reciprocal, alpha lane
/// zeroed). See the module docs for the reciprocal-multiply fidelity
/// point, the explicit little-endian decode, and the no-aliasing contract.
///
/// Degenerate dims (`width == 0` or `height == 0`) or a non-positive or
/// non-finite `max_channel` are no-ops; short buffers and a `rowbytes` narrower than
...
/// Structurally divergent reference for `avif_u16_to_float`: walks pixels
...
/// `AVIF_RGB_FORMAT_RGB` with 10- or 12-bit depth, `rowbytes` its per-row
/// byte stride) and `mipbuf` at least `4 * width * height` floats (the
...
/// `(float)((1 << bit_depth) - 1)` — 1023.0 for 10-bit, 4095.0 for
/// 12-bit — strictly positive.
#[no_mangle]
pub unsafe extern "C" fn darkroom_avif_u16_to_float(
    rgb_buf: *const u8,
    mipbuf: *mut f32,
    width: usize,
    height: usize,
...
// ── Export kernels (m4-216) ──────────────────────────────────────────────────

/// Quantize float RGB lanes into the 10/12-bit AVIF export plane.
///
/// Port of the `case 12 / case 10` branch of `write_image`
/// (`src/imageio/format/avif.c`, m4-216): `src` holds `4*width*height`
/// tightly packed floats (RGBA; the alpha lane is never read), `out`
/// receives the libavif RGB plane (`height` rows, `rowbytes` bytes each,
/// 3 host-order u16 lanes per pixel at `AVIF_RGB_FORMAT_RGB`). Per lane,
...
/// HEIF export siblings from m4-213/m4-214, whose C loops match this one
/// lane for lane):
...
///   a native-endian 16-bit store. darktable only supports little-endian
///   hosts (stated in `imageio_heif.c` next to its decode call, and libavif
///   emits host-order lanes the same way), so the kernel encodes each lane
///   with `u16::to_le_bytes` explicitly rather than relying on the host
///   endianness (mirrors the explicit `from_le_bytes` loads of the m4-209
///   import kernel).
...
/// Structurally divergent reference for `avif_float_to_u16`: walks pixels
/// by flat index with `div`/`mod` (the kernel uses nested row/column loops
...
/// least `(height - 1) * rowbytes + 6 * width` bytes (the libavif RGB
/// plane at `AVIF_RGB_FORMAT_RGB` with 10- or 12-bit depth, `rowbytes`
/// its per-row byte stride). The two buffers must not overlap.
/// `max_channel` is the C `max_channel_f`, i.e.
/// `(float)((1 << bit_depth) - 1)`, strictly positive (1023.0 at 10-bit
/// depth, 4095.0 at 12-bit depth).
#[no_mangle]
pub unsafe extern "C" fn darkroom_avif_float_to_u16(
    in_data: *const f32,
    out: *mut u8,
    width: usize,
    height: usize,
    ...
    avif_float_to_u16(src, out, width, height, rowbytes, max_channel);
}

/// Quantize float RGB lanes into the 8-bit AVIF export plane.
///
/// Port of the `case 8` branch of `write_image`
/// (`src/imageio/format/avif.c`, m4-216): `src` holds `4*width*height`
/// tightly packed floats (RGBA; the alpha lane is never read), `out`
/// receives the libavif RGB plane (`height` rows, `rowbytes` bytes each,
/// 3 u8 lanes per pixel at `AVIF_RGB_FORMAT_RGB`). Per lane,
/// `out = (u8)roundf(CLAMP(src[c] * max_channel, 0, max_channel))` with
/// `max_channel = 255.0` at 8-bit depth.
///
/// Fidelity notes (each pinned by a test below; same spelling as the
/// 10/12-bit sibling `avif_float_to_u16` above and the HEIF export
/// kernels of m4-213/m4-214):
/// - The multiply runs before the clamp, in f32, exactly as parenthesised
...
// ── Independent reference for the 8-bit export kernel ────────────────────────

/// Structurally divergent reference for `avif_float_to_u8`: walks pixels
/// by flat index with `div`/`mod` (the kernel uses nested row/column loops
/// with explicit stride arithmetic) and converts each lane through a
/// scalar helper over zipped slice iterators (the kernel indexes both
/// sides directly), so the sweep test cross-checks indexing as well as
...
/// precondition, enforced here by early return rather than clamping.
#[cfg(test)]
fn ref_lane_u8(v: f32, max_channel: f32) -> u8 {
    let scaled = v * max_channel;
    let cl = if scaled > max_channel {
        max_channel
    } else if scaled < 0.0 {
...
fn ref_avif_float_to_u8(
    ...
    for p in 0..npixels {
        ...
        let lanes: Vec<u8> = src[y * src_row + 4 * x..y * src_row + 4 * x + 3]
            .iter()
            .map(|&v| ref_lane_u8(v, max_channel))
            .collect();
...
/// `in_data` must hold at least `4 * width * height` floats (the C caller
/// passes the export pipeline buffer, tightly packed RGBA) and `out` at
/// least `(height - 1) * rowbytes + 3 * width` bytes (the libavif RGB
/// plane at `AVIF_RGB_FORMAT_RGB` with 8-bit depth, `rowbytes` its
/// per-row byte stride). The two buffers must not overlap. `max_channel`
/// is the C `max_channel_f`, i.e. `(float)((1 << bit_depth) - 1)`,
/// strictly positive (255.0 at 8-bit depth).
#[no_mangle]
pub unsafe extern "C" fn darkroom_avif_float_to_u8(
    in_data: *const f32,
    out: *mut u8,
    width: usize,
    height: usize,
    ...
    let src = std::slice::from_raw_parts(in_data, src_len);
    let out = std::slice::from_raw_parts_mut(out, dst_len);
    avif_float_to_u8(src, out, width, height, rowbytes, max_channel);
}
...
mod tests {
    ...
    // Rails for both AVIF high bit depths: 0 scales to +0.0 exactly; the
    // alpha lane is pinned to +0.0; the full-scale lane pins the
    // reciprocal-multiply spelling the C loop uses (1023 * (1/1023), not
    // 1023/1023).
    #[test]
    fn rails_pin() {
        for max in [1023.0f32, 4095.0f32] {
            let full = max as u16;
            // one pixel: lanes (0, 1, full)
            let src = [0u8, 0, 1, 0, (full & 0xFF) as u8, (full >> 8) as u8];
            let mut out = vec![7.0f32; 4];
            avif_u16_to_float(&src, &mut out, 1, 1, 6, max);
            assert_eq!(out[0].to_bits(), 0x0000_0000); // 0 -> +0.0
            assert_eq!(out[1].to_bits(), (1.0f32 * (1.0f32 / max)).to_bits());
            assert_eq!(out[2].to_bits(), (max * (1.0f32 / max)).to_bits());
    ...
    fn matches_reference_over_sweep() {
        ...
        for (w, h, rb, max) in shapes {
            ...
            avif_u16_to_float(&src, &mut direct, w, h, rb, max);
            ref_avif_u16_to_float(&src, &mut reference, w, h, rb, max);
    ...
    fn degenerate_guards_no_op() {
        ...
        avif_u16_to_float(&src, &mut out, 0, 2, 6, 1023.0);
        avif_u16_to_float(&src, &mut out, 2, 0, 12, 1023.0);
        assert_eq!(out, vec![9.0f32; 8]);
        // non-positive max: output untouched
        avif_u16_to_float(&src, &mut out, 1, 2, 6, 0.0);
        avif_u16_to_float(&src, &mut out, 1, 2, 6, -3.0);
        avif_u16_to_float(&src, &mut out, 1, 2, 6, f32::NAN);
        assert_eq!(out, vec![9.0f32; 8]);
        // rowbytes narrower than one row: no-op
        avif_u16_to_float(&src, &mut out, 2, 1, 6, 1023.0);
        assert_eq!(out, vec![9.0f32; 8]);
        // truncated source: no panic, row 0 converts fully, unwritten rows
        // stay untouched. 2x2 tight needs 24 bytes; 13 bytes hold row 0
        // (12) plus one byte.
        let short_src = vec![0x02u8; 13];
        let mut short_out = vec![0.0f32; 16];
        avif_u16_to_float(&short_src, &mut short_out, 2, 2, 12, 1023.0);
        ...
        avif_u16_to_float(&empty, &mut out2, 1, 1, 6, 1023.0);
    ...
    fn ffi_round_trip() {
        ...
        unsafe {
            darkroom_avif_u16_to_float(src.as_ptr(), ffi_out.as_mut_ptr(), w, h, rb, max);
        }
        avif_u16_to_float(&src, &mut direct_out, w, h, rb, max);
    ...
    fn ffi_guards() {
        ...
        unsafe {
            // null pointers
            darkroom_avif_u16_to_float(std::ptr::null(), out.as_mut_ptr(), 2, 2, 12, 1023.0);
            darkroom_avif_u16_to_float(src.as_ptr(), std::ptr::null_mut(), 2, 2, 12, 1023.0);
            // zero dims
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 0, 2, 12, 1023.0);
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 2, 0, 12, 1023.0);
            // non-positive / non-finite max
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, 0.0);
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, f32::NAN);
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, f32::INFINITY);
            // rowbytes narrower than one row
            darkroom_avif_u16_to_float(src.as_ptr(), out.as_mut_ptr(), 2, 2, 6, 1023.0);
            // overflowing dims
    ...
    // 0.5 lane pins the multiply-then-round spelling (0.5 * 255 = 127.5
    // -> 128), and out-of-range lanes clamp on both sides. The alpha
    // lane is never read: out holds 3 lanes per pixel.
    #[test]
    fn export_u8_rails_pin() {
        let max = 255.0f32;
        let src = [0.0f32, 0.5, 1.0, 0.9, 2.0, -0.1, 0.25, 0.0];
        let mut out = vec![0xCDu8; 6];
        avif_float_to_u8(&src, &mut out, 2, 1, 6, max);
        assert_eq!(out[0], 0);
        assert_eq!(out[1], 128); // 127.5 -> 128, round-half-away
        assert_eq!(out[2], 255);
    ...
    // Multiply-then-clamp order matters: a -0.5/255 lane scales to -0.5
    ...
    #[test]
    fn export_u8_clamp_order_and_nan() {
        let max = 255.0f32;
    ...
    #[test]
    fn export_u8_matches_reference_over_sweep() {
        // (width, height, rowbytes)
        let shapes = [
            (1usize, 1usize, 3usize),
            (3, 2, 9),
        ...
        for (w, h, rb) in shapes {
            ...
            avif_float_to_u8(&src, &mut direct, w, h, rb, 255.0);
            ref_avif_float_to_u8(&src, &mut reference, w, h, rb, 255.0);
    ...
    #[test]
    fn export_u8_degenerate_guards_no_op() {
        let src = vec![1.0f32; 8];
        let mut out = vec![0xCDu8; 6];
        // zero dims: output untouched
        avif_float_to_u8(&src, &mut out, 0, 2, 6, 255.0);
        avif_float_to_u8(&src, &mut out, 2, 0, 6, 255.0);
        assert_eq!(out, vec![0xCDu8; 6]);
        // non-positive / non-finite max: output untouched
        avif_float_to_u8(&src, &mut out, 2, 1, 6, 0.0);
        avif_float_to_u8(&src, &mut out, 2, 1, 6, -3.0);
        avif_float_to_u8(&src, &mut out, 2, 1, 6, f32::NAN);
        avif_float_to_u8(&src, &mut out, 2, 1, 6, f32::INFINITY);
        assert_eq!(out, vec![0xCDu8; 6]);
        // rowbytes narrower than one row: no-op
        avif_float_to_u8(&src, &mut out, 2, 1, 5, 255.0);
        assert_eq!(out, vec![0xCDu8; 6]);
        // truncated source: no panic, nothing written
        let short = vec![1.0f32; 3];
        avif_float_to_u8(&short, &mut out, 2, 1, 6, 255.0);
        assert_eq!(out, vec![0xCDu8; 6]);
    }

    #[test]
    fn export_u8_ffi_round_trip() {
        ...
        unsafe {
            darkroom_avif_float_to_u8(src.as_ptr(), ffi_out.as_mut_ptr(), w, h, rb, 255.0);
        }
        avif_float_to_u8(&src, &mut direct_out, w, h, rb, 255.0);
        assert_eq!(ffi_out, direct_out);
    }

    #[test]
    fn export_u8_ffi_guards() {
        let src = vec![1.0f32; 16];
        let mut out = vec![0xCDu8; 12];
        unsafe {
            // null pointers
            darkroom_avif_float_to_u8(std::ptr::null(), out.as_mut_ptr(), 2, 2, 6, 255.0);
            darkroom_avif_float_to_u8(src.as_ptr(), std::ptr::null_mut(), 2, 2, 6, 255.0);
            // zero dims
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 0, 2, 6, 255.0);
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, 0, 6, 255.0);
            // non-positive / non-finite max
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, 2, 6, 0.0);
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, 2, 6, f32::NAN);
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, 2, 6, f32::INFINITY);
            // rowbytes narrower than one row
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, 2, 5, 255.0);
            // overflowing dims
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), usize::MAX, 2, 6, 255.0);
            darkroom_avif_float_to_u8(src.as_ptr(), out.as_mut_ptr(), 2, usize::MAX, 6, 255.0);
        }
    ...
    #[test]
    fn export_u16_rails_pin() {
        let max = 1023.0f32;
        let src = [0.0f32, 0.5, 1.0, 0.9, 2.0, -0.1, 0.25, 0.0];
        let mut out = vec![0xCDu8; 12];
        avif_float_to_u16(&src, &mut out, 2, 1, 12, max);
        ...
        avif_float_to_u16(&[1.0f32, 1.0, 1.0, 0.0], &mut out12, 1, 1, 6, 4095.0);
    ...
    #[test]
    fn export_u16_clamp_order_and_nan() {
        let max = 1023.0f32;
        // one pixel: tiny-negative, NaN, and -1.0 lanes all read 0 (the
        // alpha slot is never read)
        let src = [-0.5f32 / 1023.0, f32::NAN, -1.0, 9.9];
        let mut out = vec![0xCDu8; 6];
        avif_float_to_u16(&src, &mut out, 1, 1, 6, max);
        ...
        avif_float_to_u16(&src2, &mut out2, 1, 1, 6, max);
    ...
    #[test]
    fn export_u16_matches_reference_over_sweep() {
        // (width, height, rowbytes, max_channel)
        let shapes = [
            (1usize, 1usize, 6usize, 1023.0f32),
            (3, 2, 18, 1023.0),
        ...
        for (w, h, rb, max) in shapes {
            ...
            let mut direct = vec![0xCDu8; rb * h];
            let mut reference = vec![0xCDu8; rb * h];
            avif_float_to_u16(&src, &mut direct, w, h, rb, max);
            ref_avif_float_to_u16(&src, &mut reference, w, h, rb, max);
    ...
    #[test]
    fn export_u16_degenerate_guards_no_op() {
        let src = vec![1.0f32; 8];
        let mut out = vec![0xCDu8; 12];
        // zero dims: output untouched
        avif_float_to_u16(&src, &mut out, 0, 2, 12, 1023.0);
        avif_float_to_u16(&src, &mut out, 2, 0, 12, 1023.0);
        assert_eq!(out, vec![0xCDu8; 12]);
        // non-positive / non-finite max: output untouched
        avif_float_to_u16(&src, &mut out, 2, 1, 12, 0.0);
        avif_float_to_u16(&src, &mut out, 2, 1, 12, -3.0);
        avif_float_to_u16(&src, &mut out, 2, 1, 12, f32::NAN);
        avif_float_to_u16(&src, &mut out, 2, 1, 12, f32::INFINITY);
        assert_eq!(out, vec![0xCDu8; 12]);
        // rowbytes narrower than one row: no-op
        avif_float_to_u16(&src, &mut out, 2, 1, 11, 1023.0);
        assert_eq!(out, vec![0xCDu8; 12]);
        // truncated source: no panic, nothing written
        let short = vec![1.0f32; 7];
        avif_float_to_u16(&short, &mut out, 2, 1, 12, 1023.0);
        assert_eq!(out, vec![0xCDu8; 12]);
    }

    #[test]
    fn export_u16_ffi_round_trip() {
        let (w, h, rb, max) = (9usize, 5usize, 60usize, 4095.0f32);
        let mut src = vec![0.0f32; 4 * w * h];
        for (i, v) in src.iter_mut().enumerate() {
            let k = ((i as u64).wrapping_mul(2_654_435_761) % 2001) as f32;
        ...
        let mut direct_out = vec![0xCDu8; rb * h];
        unsafe {
            darkroom_avif_float_to_u16(src.as_ptr(), ffi_out.as_mut_ptr(), w, h, rb, max);
        }
        avif_float_to_u16(&src, &mut direct_out, w, h, rb, max);
        assert_eq!(ffi_out, direct_out);
    }

    #[test]
    fn export_u16_ffi_guards() {
        let src = vec![1.0f32; 16];
        let mut out = vec![0xCDu8; 24];
        unsafe {
            // null pointers
            darkroom_avif_float_to_u16(std::ptr::null(), out.as_mut_ptr(), 2, 2, 12, 1023.0);
            darkroom_avif_float_to_u16(src.as_ptr(), std::ptr::null_mut(), 2, 2, 12, 1023.0);
            // zero dims
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 0, 2, 12, 1023.0);
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 0, 12, 1023.0);
            // non-positive / non-finite max
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, 0.0);
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, -3.0);
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, f32::NAN);
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 2, 12, f32::INFINITY);
            // rowbytes narrower than one row
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, 2, 11, 1023.0);
            // overflowing dims
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), usize::MAX, 2, 12, 1023.0);
            darkroom_avif_float_to_u16(src.as_ptr(), out.as_mut_ptr(), 2, usize::MAX, 12, 1023.0);
        }
