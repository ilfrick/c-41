//! Kernel ported from `src/imageio/imageio_qoi.c` (`dt_imageio_open_qoi`,
//! the u8-to-float normalize loop, m4-202). One kernel replaces the whole
//! loop body: each decoded QOI byte is scaled by `1/255` into the 4-channel
//! float mipmap buffer.
//!
//! What the C loop does, over flat indices `index` in `0..npixels * 4`:
//! - `mipbuf[index] = int_RGBA_buf[index] / 255.f`
//!
//! Unlike the RGBE (m4-201) and PNG loaders, every lane is scaled —
//! including alpha (the flat loop covers all `4 * npixels` bytes, so an
//! opaque `255` alpha becomes exactly `1.0`, not a preserved zero).
//!
//! Bit-exactness notes:
...
//! - Buffers: the C caller allocates two distinct buffers (`int_RGBA_buf`
//!   at `4*npixels` bytes from `qoi_decode`, `mipbuf` at `4*npixels` floats
//!   from the mipmap cache). The kernel must not be called with overlapping
//!   buffers.
//!
//! The Rust kernel is single-threaded sequential; the C loop was
//! `DT_OMP_FOR` over flat indices, but each output element reads only its
//! own source byte, so thread scheduling cannot change the result.

/// Scale decoded QOI bytes into the float mipmap buffer.
///
/// Port of the former element-wise loop in `dt_imageio_open_qoi`
/// (src/imageio/imageio_qoi.c): `src` holds `4*npixels` bytes (RGBA),
/// `out` receives `4*npixels` floats (each byte divided by 255). See the
/// module docs for the division-vs-multiply fidelity point and the
...
pub fn qoi_u8_to_float(src: &[u8], out: &mut [f32], npixels: usize) {
    let n = npixels.min(src.len() / 4).min(out.len() / 4);
    for i in 0..n {
        let s = 4 * i;
        out[s] = src[s] as f32 / 255.0f32;
        out[s + 1] = src[s + 1] as f32 / 255.0f32;
        out[s + 2] = src[s + 2] as f32 / 255.0f32;
        out[s + 3] = src[s + 3] as f32 / 255.0f32;
    }
}

// ── Independent reference implementation for bit-exactness tests ─────────────

/// Structurally divergent reference for `qoi_u8_to_float`: walks the OUTPUT
/// buffer quad-by-quad via `chunks_exact_mut` with a lane `enumerate` (the
/// kernel walks source quads with explicit index writes), deriving each
/// source byte from the running output position. The arithmetic is the same
/// `byte as f32 / 255.0` division by construction (see the module docs: the
/// reciprocal-multiply form is not bit-identical and must not be used).
/// Same clamped-fit precondition: well-formed buffers only.
#[cfg(test)]
fn ref_qoi_u8_to_float(src: &[u8], out: &mut [f32], npixels: usize) {
    let Some(need) = npixels.checked_mul(4) else {
        return;
    };
    if src.len() < need || out.len() < need {
        return;
    }
    let mut base = 0usize;
    for quad in out.chunks_exact_mut(4).take(npixels) {
        for (lane, slot) in quad.iter_mut().enumerate() {
            *slot = src[base + lane] as f32 / 255.0f32;
        }
        base += 4;
    }
}
...
/// the `qoi_decode` output for a `desc.width * desc.height` image) and
...
#[no_mangle]
pub unsafe extern "C" fn darkroom_qoi_u8_to_float(
    rgba_buf: *const u8,
    mipbuf: *mut f32,
    npixels: usize,
) {
    ...
    qoi_u8_to_float(src, out, npixels);
}
...
mod tests {
    ...
    // they pin the kernel to the C operation rather than to itself.
    #[test]
    fn rails_pin() {
        let src = [0u8, 1, 128, 255, 255, 0, 254, 253];
        let mut out = vec![7.0f32; 8];
        qoi_u8_to_float(&src, &mut out, 2);
        assert_eq!(out[0].to_bits(), 0x0000_0000); // 0 -> +0.0
        assert_eq!(out[3].to_bits(), 0x3F80_0000); // 255 -> 1.0
        assert_eq!(out[4].to_bits(), 0x3F80_0000); // 255 -> 1.0
        assert_eq!(out[5].to_bits(), 0x0000_0000); // 0 -> +0.0
        ...
        assert_eq!(out[7].to_bits(), (253u8 as f32 / 255.0).to_bits());
    }

    // every byte value at several quad positions, plus an odd length:
    // kernel and reference must agree bit-exactly on every lane.
    #[test]
    fn matches_reference_over_sweep() {
        for n in [1usize, 3, 65, 513] {
            ...
            qoi_u8_to_float(&src, &mut direct, n);
            ref_qoi_u8_to_float(&src, &mut reference, n);
    ...
    fn degenerate_guards_no_op() {
        ...
        qoi_u8_to_float(&src, &mut short_out, 2);
        let short_src = vec![200u8; 7]; // short for 2 quads
        let mut out2 = vec![0.0f32; 8];
        qoi_u8_to_float(&short_src, &mut out2, 2);
        let empty: Vec<u8> = vec![];
        let mut out3 = vec![0.0f32; 4];
        qoi_u8_to_float(&empty, &mut out3, 1);
    ...
    fn ffi_round_trip() {
        ...
        unsafe {
            darkroom_qoi_u8_to_float(src.as_ptr(), ffi_out.as_mut_ptr(), n);
        }
        qoi_u8_to_float(&src, &mut direct_out, n);
    ...
    fn ffi_guards() {
        ...
        unsafe {
            // null pointers
            darkroom_qoi_u8_to_float(std::ptr::null(), out.as_mut_ptr(), 4);
            darkroom_qoi_u8_to_float(src.as_ptr(), std::ptr::null_mut(), 4);
            // zero pixels
            darkroom_qoi_u8_to_float(src.as_ptr(), out.as_mut_ptr(), 0);
            // overflowing pixel count (4 * npixels wraps — both reject)
            darkroom_qoi_u8_to_float(src.as_ptr(), out.as_mut_ptr(), usize::MAX);
            darkroom_qoi_u8_to_float(src.as_ptr(), out.as_mut_ptr(), usize::MAX / 4 + 1);
        }
