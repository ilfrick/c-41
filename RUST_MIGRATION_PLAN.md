...
|-----|------------------|---------------------|
| ~~`colorbalancergb`~~ **DONE — m4-81..85** (`iop/colorbalancergb.rs`); **C loops replaced m4-137** (all 4 former OMP sites call `darkroom_colorbalancergb_{process,build_gamut_lut_jzazbz,checkerboard_fill,opacity_luts}` — process loop incl. mask-display checkerboard, JzAzBz LUT sampler, both GUI graph loops) | 0 | ~~Filmlight Yrg / `work_profile`~~ |
| ~~`colorreconstruction`~~ **DONE — m4-80** (`colorreconstruct.rs`, bespoke {L,a,b,weight} grid); **C loops replaced m4-138** (`darkroom_colorreconstruct_{splat,blur_line,slice}` over the `#[repr(C)]` cell buffer; splat's serial≡atomic-add-order documented) | 0 | ~~3D bilateral grid~~ |
...
| ~~Filmlight Yrg / `work_profile` callbacks~~ — **DONE** (Yrg/UCS/JzAzBz conversions in color.rs thru m4-81; pure-Rust module m4-83..85; C-loop FFI replacement m4-137) | ~~colorbalancergb~~ (done), colorin |
| Per-pixel ICC / LCMS — **DECISION (m4-86): pure-Rust matrix path, NO lcms2** (keeps the CMake-free / C-linkless build goal). Scope of the 20 remaining OMP loops re-surveyed: **colorout matrix path already Rust** (`darkroom_colorout_*`); **portable & DONE**: ~~colorin `:777` cmatrix-bm~~ **PORTED — m4-86** (`b0f20649e4`, `darkroom_colorin_cmatrix_bm`: tone-curve LUT + `_apply_blue_mapping` + matrix→Lab, incl. the nmatrix/lmatrix gamut-clip variant; architect APPROVE, transpose-equiv proven, full-c `-Werror` clean). So the meaningful portable-matrix ICC path is complete. **retouch ×2 (`:3082`/`:3135`) are GUI-ONLY** (re-checked m4-86 f/u): `rt_process_stats` runs only under `g && dt_pipe_is_full` w/ `preview_auto_levels==1`; `rt_adjust_levels` only when `dwt_p->return_layer>0` (the `display_wavelet_scale` toggle) — the wavelet-scale-preview auto-levels, not core processing. Reclassify as GUI-only (like colorequal/toneequal). **stays unsupported** (needs LCMS for non-matrix/LUT profiles) = colorin `:1054`/`:1097` "general lcms2 fallback", colorout's generic path. These are FFI-boundary ports (replace the C loop with a `#[no_mangle] extern "C"` call like `darkroom_colorin_cmatrix_fastpath_simple`), so they need C edits + the **full-app Docker build** (not cargo-only). **RESOLUTION (user chose option b): build a pure-Rust ICC engine** (`c41-core::icc`) to remove the lcms2 dependency entirely — same functionality (matrix **and** cLUT profiles), accuracy ≥ LCMS (f32 throughout vs LCMS's 16-bit path), pure Rust. NB: the shipped `c41-rs` product is ALREADY lcms-free (0 refs in `crates/`); the ~50 `cms*` calls live only in the transitional C darktable (colorin/colorout), of which only 7 are `cmsDoTransform` (the loops) — the rest are profile parse/build. So the engine is judged on *spec-correctness*, not bit-exact-to-LCMS. **ICC-engine progress:** m4-89 parser ✅ (`c8fd82c4e5`, APPROVE-WITH-FIXES incl. a real DoS guard); m4-90 cLUT **N-D interpolation core** (`icc/clut.rs`: LCMS-matched tetrahedral for 3-in RGB + general N-linear) ✅ (`7037102b86`, **APPROVE** — tetra vertex table verified arm-by-arm vs LCMS); m4-91/92 LUT **tags** (`mft1`/`mft2` v2, `mAB `/`mBA ` v4) parsed into `Pipeline`s of curve/matrix/CLUT stages, validate-before-reserve carried ✅ (`04d1e6217b`, `cf50d7761f`, hardening `ca546c4612`); m4-93a/b device→PCS ([`a2b_pipeline`] with appended raw-value PCS decode; matrix-shaper branch) ✅ (`cb1093a9bc`, `4f3733044d`/`2fcca538fe`); **m4-127 PCS→device + full assembly ✅**: `Profile::b2a_pipeline` (prepends the exact-algebraic ICC-*encode* stage for LUT tags; matrix-shaper fallback = adjugate-inverse colorants + new sampled/bisection `Curve::inverse`; `B2A{intent}` preference mirroring A2B) and `icc::transform::Transform` (head → optional absolute-intent white-ratio scaling in XYZ → Lab↔XYZ bridge → tail; abs degrades to relative when `wtpt` missing — documented); **m4-129 slice 1 FFI boundary + band path ✅**: allocation-free 3-channel eval (`Pipeline::eval_into3` / `Transform::eval_into`, bit-exact to the allocating paths) and the `darkroom_icc_transform_{new,free,apply_rgba}` exports (stride-4 RGBA + alpha passthrough, in-place-safe like colorin's `cmsDoTransform(out,out)` callsites; NULL-on-failure contract; intent>3 refused; non-3-channel GRAY/CMYK pipelines rejected at assembly — reviewer-caught: the parser alone accepts them and a GRAY handle would have panicked across FFI on first pixel). **m4-129 slice 2 call sites ✅**: colorin/colorout serialize their profiles once per `commit_params` (double `cmsSaveProfileToMem` into g_malloc'd buffers — builtin profiles are LCMS-API handles with no embedded bytes) and try `darkroom_icc_transform_new` first at every former `cmsCreateTransform` site, falling back per-handle to the exact previous LCMS call whenever the engine returns NULL; every `cmsDoTransform` routes through an `_icc_apply(rs, &lcms, …)` helper. Softproof/BPC/gamutcheck *and* an explicit `force_lcms2` preference stay LCMS-only by construction (`rs_xform` gated on NORMAL && !force_lcms2). Engine enablers landed with it: identity pipelines for colour-space-conversion profiles (`data_space == pcs`, e.g. darktable's bare Lab link profile — spec-correct, pinned by tests) and refusal of odd device-space signatures (device neither RGB/XYZ nor PCS-equal → NULL → LCMS fallback instead of plausible garbage from the XYZ shaper path; review MINOR). Review MAJORs fixed en route: colorin `init_pipe` is malloc-not-calloc and had to initialize all nine new fields explicitly (first `commit_params` runs `_colorin_rs_cleanup` unconditionally — garbage fields meant a wild free on the very first pipe build); the `force_lcms2` export preference was silently overridden until gated. Found en route: the 2026-08-12 rename had broken the full-c CMake build outright (`src/CMakeLists.txt` still pointed at crates/darkroom-core + libdarkroom_core.so + `-p darkroom-core`; CI's docker-full-c job is workflow_dispatch-only so nobody hit it since) — repaired alongside (75 DEPENDS paths, package flag, cdylib name/SONAME/IMPORTED_SONAME, internal target names). Remaining: retire the lcms2 dependency entirely (later increment). | colorin, colorout, retouch |
