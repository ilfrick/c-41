...
| Unit tests passing | **429** |
| IOP `.rs` files | 93 (one per C IOP) |
| Shared modules | `color`, `math`, `raw`, `geometry` |
| Last patch | `Phase 2z+78` (segbased highlight reconstruction — all 13 hlreconstruct/segbased.c loops ported; hlreconstruct/ fully done) |
...
| `highlights` | 4 | highlights.c: `interpolate_color_xtrans` / `interpolate_color` CFA inpaint. hlreconstruct/ fully migrated (lch.c, opposed.c, segmentation.c, laplacian.c, segbased.c — 0 DT_OMP_FOR left) |
