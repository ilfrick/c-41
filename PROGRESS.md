...
## 2026-08-25 20:54 UTC — m4-143: shared per-path inflight decode dedupe (grid + canvas)

**What:** m4-141 review N4 follow-up. The zoomable canvas had a private `path → bucket` in-flight map, the grid had nothing: two cells rebind-scrolling onto the same fresh raw could each spawn a seconds-long demosaic once. Rather than giving the grid its own copy, both consumers now register in ONE shared map in `thumbs` (`inflight_register`/`inflight_unregister`, main-thread `thread_local RefCell<HashMap<String,u32>>`): an equal-or-bigger bucket already registered for a path refuses the new request; a strictly bigger request supersedes; release is owner-checked (`(path,bucket)` must still match), so superseded decodes can't evict their successor and all supersede interleavings terminate with an empty map. `CanvasState.inflight` deleted; `spawn_decode` and `ensure_grid_thumb` share identical claim semantics (register → permit → hand back registration if gate busy). 3 new tests pin refusal/stale-release/supersede/per-path isolation.

**Senior review** (fricktrade-architect API-402'd as usual; fresh general-purpose agent, model opus, explicit READ-ONLY MANDATE — same documented substitution as every increment): verdict APPROVE, no blockers, with one MAJOR required this increment. **MAJOR-1 (fixed)**: the refusal path scheduled nothing — unlike the busy gate's 150ms retry — so a bind refused by a CROSS-SURFACE owner (e.g. canvas decode running while the user flips back to file-manager and scrolls) stranded blank cells until an unrelated rebind: the owner's completion lands in the other surface's cache and paints nothing here. Fix: grid refusal now schedules the same 150ms `ensure_grid_thumb` retry; canvas refusal schedules a one-shot 150ms `queue_draw`; both self-terminating. **MINOR-1 (fixed)**: canvas released its registration after the result match instead of before (a future panicking arm would leak the entry permanently); moved to mirror the grid. **MINOR-2 (fixed, doc honesty)**: module doc claimed cross-surface pixel-cache hits ("cache hit when the canvas opens the same folder") — false, the canvas keeps its private pixbuf cache; corrected, and publishing completions into one shared cache recorded as the follow-up that would also close the sequential double-demosaic. **NITs**: reverse-supersede interleaving pinned (bigger completes first, stale smaller release lands late); test-comment phrasing fixed; RAII guard for the register/unregister pairing noted as hardening follow-up. Reviewer also independently verified: no registration leak on any exit path, panic-in-decode releases registry+gate via JoinError→Err, register/unregister atomicity on the main loop, no semantic drift vs old ordering (`git show HEAD` compared).

**Verification:** 20/20 thumbs release tests (incl. new); full `scripts/ci-local.sh` GATE_EXIT=0 after the review fixes (log target/cbuild/gate-m4-143.log) — judged by exit code, not output greps.

**Follow-ups recorded:** publish decode results into one cross-surface cache (kills sequential double-demosaic + gives refusals a fast path); RAII InflightGuard making leak-by-future-edit structurally impossible; embedded-preview-first raw-thumb path still open.

## 2026-08-25 21:41 UTC — m4-144: lighttable multi-image selection + actions on the selection

**What:** darktable's lighttable click model — more than one image selectable at once, actions fan out over all of them. New `lighttable::selection` module holds the set as PATH-keyed session state (paths survive reloads/culling-window slides; placeholders carry no `/`), while the grid keeps its `SingleSelection` cursor — upstream's active-image-vs-selection split, chosen so no existing consumer (preview stepping, darkroom entry, metadata panel, culling) churns. Interactions: plain click = exclusive select (+anchor), ctrl-click = toggle, shift-click = inclusive range from the anchor either direction (stale anchor degrades to target-only; unknown target changes nothing), ctrl+A over the culling BASE collection, Esc clears. Selected cells get an inset outline (`c41-cell-selected`) re-decided on every bind plus a walker that repaints realized cells on mutation; a hidden bottom-bar count label ("N images selected") appears only when non-empty. The F-key colour and digit-rating actions now fan out over the snapshot with cursor fallback (colour per-image bit toggle semantics — mixed masks are faithful; ratings absolute set). Reload sweep: `prune_to` in `fill_grid`, the one fill every loader shares.

**Senior review** (fricktrade-architect API-402'd as usual; fresh general-purpose agent, model opus, explicit READ-ONLY MANDATE — same documented substitution as every increment): verdict **BLOCK**, 1 blocker + 3 majors + 3 minors + NITs; all fixed this increment. **BLOCKER-1**: the ctrl+A arm landed AFTER `handle_lt_key`'s first-statement modifier bail-out — unreachable dead code advertising a dead key (and the comment claimed an ordering that didn't exist). Fix: arm hoisted above the bail-out, self-guarding shift/alt; deliberately before the preview handler too (documented: preview has no use for the key). **MAJOR-2**: the prune "chokepoint" comment was false — `reselect_path` only sees tag-filter reapply and culling enter/leave; search/rating/colour/sort/folder/import reloads splice via `fill_grid` unpruned → inconsistent selection survival across surfaces, stale count labels, silent keyboard writes to invisible images. Fix: sweep moved into `fill_grid`; `reselect_path` back to cursor-restoration only. **MAJOR-3**: both fan-outs ran the whole batch under ONLY the anchor's `path_write_lock`, voiding the per-path serialization invariant for N−1 targets — a dot-click on selected image #3 could interleave with the batch's read-modify-write colour toggle (get→set/remove across statements, no transaction). Fix: sequential per-target `serialized_write` loops (own lock each; panic scope now per-image; N blocking-pool hops is nothing at input rates). **MAJOR-4**: Escape gated on `state.is_empty()` — dead under NumLock/CapsLock (MOD2/LOCK ride in the event state) for exactly the keypad users `digit_to_rating` documents support for. Fix: intersects-style predicate ignoring lock bits, matching the bail-out's own form. **MINOR-5**: gesture accepted every mouse button (right-click rewrote the selection) → `set_button(BUTTON_PRIMARY)`. **MINOR-6**: zoomable canvas never repainted on set-only mutations (Esc while canvas up left stale frames indefinitely) → canvas registered via WeakRef, `notify_changed` queue_draws it. **MINOR-7**: cell action fired at PRESS before drag disambiguation → moved to RELEASED like the canvas handler. **NITs fixed**: stale doc referenced nonexistent `lighttable::selection_changed`; `select_all`'s anchor-clearing pinned in a test; cursor-frame-vs-count-label divergence documented at the zoomable paint site; rating repaints now gated on committed writes. Reviewer also verified safe: gesture never claims sequences (native select/double-click-activate intact), star/dot rows not descendants of the thumb gesture, anchor-staleness handling, WeakRef lifecycle leak-free, Ctrl+A scope correct against the base model, Esc can't steal from the search entry (outside both controller subtrees).

**Went wrong:** two Write-tool generations of selection.rs came out corrupted mid-file (placeholder tokens duplicated — same long-file failure shape seen before); bash heredoc composition produced the clean 350-line module instead. First full gate ran green PRE-review-fixes and was re-run after them — the pre-fix GATE_EXIT=0 did not count for shipping.
...
**What.** Ported only the `dt_imageio_open_rgbe` clamp-and-pack loop
(`src/imageio/imageio_rgbe.c`), replacing it with `darkroom_rgbe_clamp_pack`.
Added new `c41-core::rgbe` (safe kernel, divergent reference, validated FFI,
5 tests — `[0,10000]` rails with explicit NaN→10000 arm, `-0`→`+0`, alpha
`+0.0`, no-alias contract); registered `pub mod rgbe`; declared the export in
`src/rust_ffi/darkroom_core.h` after the PFM precedent. Decode, alloc, and
color-matrix orchestration untouched; single call site. Left all unrelated
files untouched.

**Review.** Independent senior-reviewer agent: **APPROVE-WITH-FIXES**, P2-only.
Fixed all three in-session (`#[allow(dead_code)]` → `#[cfg(test)]` on the
reference; header guard-contract sentence completed; sequential-vs-OMP note
already present in module docs). The dev session never compiled — the gate
below is the first compile proof and it passed.
...
## 2026-09-16 09:10 UTC — m4-214: port HEIF u16 export loop to Rust FFI

**Commit** `f7bc9274c2` (GitHub + Gitea via `git push origin master`)

**What.** Ported only the `case 10/12` export loop of `write_image`
(`src/imageio/format/heif.c`), replacing it with `darkroom_heif_float_to_u16`.
Extended `c41-core::heif` (safe kernel, divergent reference, validated FFI,
6 tests — multiply-before-clamp in f32, glib CLAMP order, NaN falls through
to round then saturates to 0, half-away rounding, explicit LE encode, alpha
never read); declared the export in `src/rust_ffi/darkroom_core.h` after the
u8 block. `format/heif.c` now has zero `DT_OMP_FOR` sites. Left all unrelated
files untouched.

**Review.** Independent senior-reviewer agent: **APPROVE-WITH-FIXES**, one P1.
Fixed in-session: stale "10/12-bit branch stays in C" line in the u8 header
block repointed at m4-214; LE justification reworded to cite the explicit
`RRGGBB_LE` chroma (no endianness statement exists in `format/heif.c`);
kernel-vs-ref short-buffer divergence documented (partial-write vs no-write,
well-formed-only comparison); negative-max FFI guard line added.
...
Remote CI on `fac23b65c2` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `d0dde43234` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `726265d42c` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `45351f5620` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `62f7990586` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `baed244253` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
403 passed.
Remote CI on `4e5d445fea` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
Remote CI on `1acb67d837` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
existing + 5 new), 0 failed; release `c41-ui` suite 403 passed.
Remote CI on `a1458eeff6` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `ca3ac89a62` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
0 failed; release `c41-ui` suite 403 passed.
Remote CI on `3040e04758` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
suite 403 passed.
Remote CI on `243c7bccdc` is green: `check + test + clippy`, `CMake + Rust workspace, Ubuntu 24.04 (Release, GCC)`
(which covers the changed-C Release `-Werror` compile), and `Build & push
Docker image` all `success`; matrix/full-c jobs skipped as expected. Both
...
Remote CI on `e7150e8e7a` is green: `check + test + clippy` and `Build & push
Docker image` both `success`; matrix/full-c jobs skipped as expected.
