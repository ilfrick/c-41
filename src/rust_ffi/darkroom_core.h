...
// PNM PPM 8-bit RGB-triplet normalize (imageio_pnm.c, dt_imageio_open_pnm,
// m4-230).
//
// darkroom_pnm_ppm_u8_row_to_float replaces the inner per-pixel loop of
// the max <= 255 branch of _read_ppm: per column x,
// out[4*x + c] = (float)line[3*x + c] / (float)max for c in 0..2, and
// out[4*x + 3] = 0.0 (the explicitly zeroed alpha lane). Each RGB lane
// reads its own triplet byte (no fan-out, unlike the gray PGM sibling).
// The division is replayed per lane, not via a precomputed reciprocal;
// the denominator conversion is hoisted once, which is bit-identical
// because max is loop-invariant and the conversion is exact. The row
// fread, the line allocation, and the PBM/PGM/16-bit sibling branches
// stay in C. out holds 4*width floats, line holds 3*width bytes; null
// pointers, a zero width, a zero max, and an overflowing 4*width or
// 3*width product are guarded no-ops; the buffers must not overlap.
void darkroom_pnm_ppm_u8_row_to_float(float *out, const unsigned char *line,
                                      size_t width, unsigned int max);

// PNM PPM 16-bit RGB-triplet normalize (imageio_pnm.c, dt_imageio_open_pnm,
// m4-231).
//
// darkroom_pnm_ppm_u16_row_to_float replaces the inner per-pixel loop of
// the max > 255 branch of _read_ppm: per column x, each line triplet word
// is byte-swapped from big-endian file order (PPM stores the most
// significant byte first; the C loop swaps under
// G_BYTE_ORDER != G_BIG_ENDIAN and darktable supports only little-endian
// hosts (src/imageio/imageio_heif.c:167), so the swap is
// unconditional), then out[4*x + c] = (float)decoded / (float)max for c in
// 0..2, and out[4*x + 3] = 0.0 (the explicitly zeroed alpha lane). Each
// RGB lane reads its own triplet word (no fan-out, unlike the gray PGM
// sibling). The division is replayed per lane, not via a precomputed
// reciprocal; the denominator conversion is hoisted once, which is
// bit-identical because max is loop-invariant and the conversion is exact.
// The row fread, the line allocation, and the PBM/PGM/8-bit sibling
// branches stay in C. out holds 4*width floats, line holds 3*width words;
// null pointers, a zero width, a zero max, and an overflowing 4*width or
// 3*width product are guarded no-ops; the buffers must not overlap.
...
// and an overflowing 4*width or 3*width product are guarded no-ops; the
// buffers must not overlap.
void darkroom_jpeg_rgb24_row_to_rgba(unsigned char *tmp, const unsigned char *row,
                                     size_t width);
...
int darkroom_imageio_has_mono_rgbx(const unsigned char *inp, int32_t width, int32_t height);
...
// TIFF chunky float row copy (imageio_tiff.c, _read_chunky_f, m4-240).
//
// darkroom_tiff_chunky_f_row_to_float replaces the inner per-pixel loop of
// _read_chunky_f: per column i, out[4*i] = in[spp*i], then the spp < 3
// mono-splat branch (out[4*i+1] = out[4*i+2] = out[4*i], so scanline
// lanes 1 and up are never read) or the color branch
// (out[4*i+1] = in[spp*i+1], out[4*i+2] = in[spp*i+2]), then
// out[4*i+3] = 0.0 (the explicitly zeroed alpha lane: the file alpha
// lane in[spp*i+3], present when spp == 2 or spp >= 4, is never read).
// Pure float copies, no arithmetic, no scaling. The per-row
// TIFFReadScanline, the row setup, the cursor advance, and the Lab
// cmsDoTransform sibling variants stay in C. out holds 4*width floats,
// in holds spp*width floats; null pointers, a zero width, a zero spp,
// and an overflowing 4*width or spp*width product are guarded no-ops;
// the buffers must not overlap.
...
// TIFF chunky half-float row convert (imageio_tiff.c, _read_chunky_h, m4-243).
//
// darkroom_tiff_chunky_h_row_to_float replaces the inner per-pixel loop of
// _read_chunky_h: per column i, r = half_to_float(in[spp*i]) (the
// production spelling is imath_half_to_float: -DHAVE_IMATH is in the
// imageio_tiff.c flags in build/compile_commands.json, so the Imath 3.1 C
// API conversion runs; without Imath the local rygorous bit-trick
// _half_to_float runs instead — both are the canonical exact binary16 to
// binary32 widening (exact for every non-NaN input; NaN payloads shift
// through unquieted in both spellings, verified exhaustively), replicated
// bit-exactly in Rust), then out[4*i] = r
// (NO invert: _read_chunky_h takes no photometric argument, unlike
// _read_chunky_8, so lane 0 is the converted r and is never flipped — a
// genuine divergence from the m4-241 u8 sibling), then the spp < 3
// mono-splat branch (out[4*i+1] = out[4*i+2] = out[4*i], so scanline lanes
// 1 and up are never read) or the color branch (out[4*i+1] from
// in[spp*i+1], out[4*i+2] from in[spp*i+2], converted but never inverted),
// then out[4*i+3] = 0.0 (the explicitly zeroed alpha lane: the file alpha
// lane in[spp*i+3], present when spp == 2 or spp >= 4, is never read).
// Endianness: in is the uint16_t scanline buffer from TIFFReadScanline,
// which libtiff returns in host byte order — native u16 lanes, NO
// byte-swap (same as the m4-242 u16 sibling). The per-row
// TIFFReadScanline, the row setup, the cursor advance, and the Lab
// cmsDoTransform sibling variants stay in C. out holds 4*width floats, in
// holds spp*width uint16_t lanes; null pointers, a zero width, a zero spp,
// an overflowing 4*width or spp*width product, and either byte span
// exceeding isize::MAX are guarded no-ops; the buffers must not overlap.
void darkroom_tiff_chunky_h_row_to_float(float *out, const uint16_t *in,
                                         size_t width, size_t spp);
...
/*
 * HEIF 16-bit RGB normalize (imageio_heif.c, dt_imageio_open_heif, m4-208).
 *
 * darkroom_heif_u16_to_float replaces the former DT_OMP_FOR_SIMD pixel
 * loop: per pixel (y, x), out[4*(y*width+x) + c] = (float)in_pixel[c] *
 * (1.0f / max_channel) for c in 0..2, where in_pixel is the little-endian
 * u16 triple at byte offset y*rowbytes + 6*x of the interleaved RRGGBB_LE
 * plane, and out[4*(y*width+x) + 3] is zeroed. Input rows may carry
 * padding (rowbytes >= 6*width); output rows are tightly packed. The
 * libheif decode, the mipmap-cache allocation, and the colorspace and
 * flag setup stay in C. rgb_buf holds (height-1)*rowbytes + 6*width
 * bytes, mipbuf 4*width*height floats; max_channel is the C
 * max_channel_f ((float)((1 << decoded_values_bit_depth) - 1)). Null
 * pointers, zero dims, a non-positive max_channel, a rowbytes narrower
 * than one pixel row, and overflowing dim products are guarded no-ops;
 * the buffers must not overlap.
 */
void darkroom_heif_u16_to_float(const unsigned char *rgb_buf, float *mipbuf,
 ...
 * m4-209).
 *
 * darkroom_avif_u16_to_float replaces the former DT_OMP_FOR_SIMD pixel
 * loop of the case 12 / case 10 branch: per pixel (y, x),
 * out[4*(y*width+x) + c] = (float)in_pixel[c] * (1.0f / max_channel) for
 * c in 0..2, where in_pixel is the u16 triple at byte offset y*rowbytes +
 * 6*x of the libavif RGB plane (AVIF_RGB_FORMAT_RGB at 10- or 12-bit
 * depth, host-order lanes read as little-endian), and
 ...
 * non-finite max_channel, a rowbytes narrower than one pixel row, and
 ...
 * (x, then y axes). Null pointers, zero sizes, and overflowing offset
 * bounds are guarded no-ops; the stated buffer length remains a caller
 * contract.
 */
...
/*
 * Bilateral-grid blur-line z derivative (src/common/bilateral.c, m4-199).
 *
 * darkroom_bilateral_blur_line_z replaces the blur_line_z pass of
 * dt_bilateral_blur(): the -2-derivative-of-gaussian pass along offset3
 * (asymmetric forward/backward edge handling), completing the Rust port of
 * the blur. Same guard contract as darkroom_bilateral_blur_line.
 */
void darkroom_bilateral_blur_line_z(float *buf,
                                     size_t offset1, size_t offset2, size_t offset3,
                                     size_t size1, size_t size2, size_t size3);
...
/*
 * TIFF 8-bit grayscale detection (src/imageio/format/tiff.c, write_image,
 * m4-219).
 *
 * darkroom_tiff_u8_is_grayscale replaces the former DT_OMP_FOR pixel loop
 * of the 8bpp shortfile branch: returns 1 when every interior pixel has
 * its R/G/B lanes pairwise within 2 (abs diffs), 0 at the first pixel
 * that differs. The 1-pixel border is never read (pipeline edge artefacts
 * stay excluded, as in C), and the alpha lane is never read. The C flag
 ...
 * and dims gates and the TIFF field writes stay in C (the 16-bit sibling
 * scan is `darkroom_tiff_u16_is_grayscale` below, the float sibling
 * `darkroom_tiff_f32_is_grayscale` further below). Null pointers,
 ...
 * shortfile and dims gates, and the TIFF field writes stay in C (the
 * float sibling scan is `darkroom_tiff_f32_is_grayscale` below). Null
 * pointers, degenerate dims, and overflowing dim products are guarded
 * no-ops returning 1.
 */
...
/*
 * TIFF float grayscale detection (src/imageio/format/tiff.c, write_image,
 * m4-221).
 *
 * darkroom_tiff_f32_is_grayscale replaces the former DT_OMP_FOR pixel loop
 * of the 32bpp / 16-bit-float shortfile branch: returns 1 when every
 * interior pixel has its 0.001-clamped R/G/B lane ratios within 1.01
 * (absolute value), 0 at the first pixel that differs. The 1-pixel border
 * is never read (pipeline edge artefacts stay excluded, as in C), and the
 * alpha lane is never read. The C flag was written idempotently (1 -> 3
 * only, never back), so the OpenMP schedule could not change the outcome
 * and the serial scan is result-identical. in holds 4*width*height float
 * lanes (RGBA). The shortfile and dims gates and the TIFF field writes
 * stay in C. Null pointers, degenerate dims, and overflowing dim products
 * are guarded no-ops returning 1.
 */
int darkroom_tiff_f32_is_grayscale(const float *in, size_t width,
                                   size_t height);
