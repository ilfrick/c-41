                                             ...
                                             const dt_aligned_pixel_t coeffs)
{
  ...
  // (NOTE: dt_colorspaces_cygm_apply_coeffs_to_rgb currently has no callers)
  darkroom_colorspaces_cygm_apply_coeffs(out, in, (size_t)num,
                                         (const double *)RGB_to_RGB_WB);
}
...
{
  // Ported to Rust FFI, replaces the former element-wise loop
  darkroom_colorspaces_cygm_to_rgb(out, (size_t)num, (const double *)CAM_to_RGB);
}
...
{
  // Ported to Rust FFI, replaces the former element-wise loop (the one
  // carrying the FIXME about the i*3 stride / 4-float write; sequential,
  // like the C serial fallback)
  darkroom_colorspaces_rgb_to_cygm(out, (size_t)num, (const double *)RGB_to_CAM);
}
