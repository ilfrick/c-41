                                ...
                                const dt_iop_channelmixer_rgb_version_t version)
{
  ...
  darkroom_channelmixerrgb_loop_switch(in, out, width * height,
      (const float *)RGB_to_XYZ_trans, (const float *)RGB_to_LMS_trans,
      (const float *)MIX_to_XYZ_trans, (const float *)XYZ_to_RGB_trans,
      minval, illuminant, saturation, lightness, grey,
      p, gamut, (int)clip, (int)apply_grey, (int)kind, (int)version);
}
