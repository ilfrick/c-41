...
static dt_imageio_retval_t _read_ppm(dt_image_t *img, FILE*f, float *buf)
{
  ...
  else
  {
    ...
    for(size_t y = 0; line && y < img->height; y++)
    {
      ...
      // 16-bit RGB-triplet normalize loop ported to Rust FFI (m4-231): each
      // line triplet byte-swaps from big-endian file order and scales by
      // 1/max into the RGB lanes of the 4-channel float mipmap row, alpha
      // lane zeroed, as before. The row fread above stays in C.
      darkroom_pnm_ppm_u16_row_to_float(buf_iter, line, (size_t)img->width, max);
      buf_iter += (size_t)4 * img->width;
    }
    free(line);
  }
